function hide_previous_attempts() {
    const titles = document.getElementsByClassName('title');
    for (let i = 0; i < titles.length; ++i) {
        if (i > 0 && titles.item(i).textContent == titles.item(i - 1).textContent) {
            // Previous row should be marked hidden as hidden since rows with the same title are in
...
document.getElementById('chk-only-latest').onchange = (e) => {
    if (e.target.checked) {
        ...
        document.querySelectorAll('tr.hidden').forEach((row) => row.classList.remove('hidden'));
    }
}
